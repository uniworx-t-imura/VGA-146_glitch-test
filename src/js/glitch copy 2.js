...
// ■ image(p5.Imageオブジェクト, x, y)　　
// p5.Imageオブジェクトを(x,y)の位置に描きます。(x,y)が画像の左角か中央かは、imageMode()の指定で決まります。デフォルトではx,yは画像の左上角の位置。
class Glitch {
  constructor(img, p) {
    this.p = p;
    this.channelLen = 4;
  ...
  show() {
    ...
    let n = this.p.floor(this.p.random(90));
    if (n > 75 && this.throughFlag) {
      this.throughFlag = false;
      setTimeout(() => {
        this.throughFlag = true;
      }, this.p.floor(this.p.random(40, 5000)));////グリッチが起きるかチェック
    }
    if (!this.throughFlag) {
      this.p.push();
      // this.p.translate(
      ...
      // );
      // this.p.imageMode(CORNER);
      // let w = this.p.width;
      // let h = this.p.height * (this.p.width / this.imgOrigin.height);
      ...
      // this.p.image(this.imgOrigin, 0, 0,w,h);
    ...
    this.shiftRGBs.forEach((v, i, arr) => {
      if (this.p.floor(this.p.random(100)) > 65) {//頻度
    ...
    this.scatImgs.forEach((obj) => {
      ...
      if (this.p.floor(this.p.random(100)) > 80) {//頻度はここっぽい
        obj.x = this.p.floor(
